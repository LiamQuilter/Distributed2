...
public class MapReduceFiles {
    ...
    // Added configuration parameters for testing
    private static final int[] MAP_LINES_OPTIONS = {1000, 2000, 5000, 10000};
    private static final int[] REDUCE_WORDS_OPTIONS = {500, 1000, 2000, 5000};
    private static final int DEFAULT_MAP_LINES = 2000;
    private static final int DEFAULT_REDUCE_WORDS = 1000;
    ...
    public static void main(String[] args) {
        ...
        // APPROACH #1: Brute force (unchanged)
        {
        ...
        // APPROACH #2: MapReduce (unchanged)
        {
            long startTime = System.currentTimeMillis(); // Start timing
        ...
        // APPROACH #3: Distributed MapReduce (completely unchanged)
        {
            long startTime = System.currentTimeMillis(); // Start timing
        ...
        // New testing code added after original implementations
        System.out.println("\n=== Starting Parameter Tests ===");
        runParameterTests(input);
    }

    // New testing methods added below original code
    private static void runParameterTests(Map<String, String> input) {
      // Run baseline test
      System.out.println("\n[Baseline Test]");
      Map<String, Long> baseline = runParameterizedVersion(input, DEFAULT_MAP_LINES, DEFAULT_REDUCE_WORDS);
  
      // Test different map configurations
      System.out.println("\n=== Testing Map Phase ===");
      for (int lines : MAP_LINES_OPTIONS) {
          if (lines == DEFAULT_MAP_LINES) continue;
          System.out.println("\nTesting with " + lines + " lines per map thread:");
          Map<String, Long> results = runParameterizedVersion(input, lines, DEFAULT_REDUCE_WORDS);
          printComparison(baseline, results);
      }
  
      // Test different reduce configurations - FIXED SINGLE EXECUTION
      System.out.println("\n=== Testing Reduce Phase ===");
      for (int words : REDUCE_WORDS_OPTIONS) {
          if (words == DEFAULT_REDUCE_WORDS) continue;
          System.out.println("\nTesting with " + words + " words per reduce thread:");
          Map<String, Long> results = runParameterizedVersion(input, DEFAULT_MAP_LINES, words);
          printComparison(baseline, results);
      }
  }

    private static Map<String, Long> runParameterizedVersion(Map<String, String> input, 
                                                          int linesPerMapThread,
                                                          int wordsPerReduceThread) {
        ...
        for (Map.Entry<String, String> entry : input.entrySet()) {
            ...
            for (String line : lines) {
                ...
                if (currentChunk.size() >= linesPerMapThread) {
                    chunks.add(currentChunk.toArray(new String[0]));
                    currentChunk.clear();
                }
            }
        ...
        List<Thread> mapThreads = new ArrayList<>(chunks.size());
        for (String[] chunk : chunks) {
            Thread t = new Thread(() -> {
                ...
                map("chunk", contents.toString(), mapCallback);
            });
            mapThreads.add(t);
            t.start();
        }
        ...
        for (MappedItem item : mappedItems) {
            groupedItems.computeIfAbsent(item.getWord(), k -> new LinkedList<>()).add(item.getFile());
        }
        ...
        for (Map.Entry<String, List<String>> entry : entries) {
            currentChunk.add(entry);
            if (currentChunk.size() >= wordsPerReduceThread) {
                reduceChunks.add(currentChunk);
                currentChunk = new ArrayList<>();
            }
        }
        ...
        List<Thread> reduceThreads = new ArrayList<>(reduceChunks.size());
        for (List<Map.Entry<String, List<String>>> chunk : reduceChunks) {
            Thread t = new Thread(() -> {
                for (Map.Entry<String, List<String>> entry : chunk) {
        ...
        timings.put("map", mapEnd - mapStart);
        timings.put("group", groupEnd - groupStart);
        timings.put("reduce", reduceEnd - reduceStart);
        timings.put("total", System.currentTimeMillis() - startTime);
        
        System.out.println("Test Results - Map: " + timings.get("map") + "ms, " +
                         "Reduce: " + timings.get("reduce") + "ms, " +
                         "Total: " + timings.get("total") + "ms");
        
        return timings;
    }

    private static void printComparison(Map<String, Long> baseline, Map<String, Long> current) {
      System.out.println("Map Phase:    " + current.get("map") + "ms (" 
          + getComparisonText(baseline.get("map"), current.get("map")) + ")");
      System.out.println("Group Phase:  " + current.get("group") + "ms (" 
          + getComparisonText(baseline.get("group"), current.get("group")) + ")");
      System.out.println("Reduce Phase: " + current.get("reduce") + "ms (" 
          + getComparisonText(baseline.get("reduce"), current.get("reduce")) + ")");
      System.out.println("Total Time:   " + current.get("total") + "ms (" 
          + getComparisonText(baseline.get("total"), current.get("total")) + ")\n");
  }

    private static String getComparisonText(long baseline, long current) {
        if (current < baseline) {
            return "faster by " + (baseline - current) + "ms";
        } else if (current > baseline) {
            return "slower by " + (current - baseline) + "ms";
        }
        return "no change";
    }
